...
* @author: Michael Orji
*/
if(!class_exists('DirectoryInspector'))
{
	...
	*/
	...
	public static function get_directory_contents($directory, $mode = 'BOTH', $recursive = false){
		...
		switch($mode){
			...
			default                 : self::set_initial_supplied_directory($directory); return array_merge(self::get_directory_directories($directory, $recursive), self::get_directory_files($directory));break;

		}
	}

	// A function to create an array of all the files in target folder.
	protected static function get_directory_files($folder){ //TO DO: add a second parameter to determine the types of files to get
