...
class Paginator{
	...
	protected function determine_number_of_pages(){
   		...
   		if (isset($_GET['num_pages']) && is_numeric($_GET['num_pages']) && ($_GET['num_pages'] > 0) ) { // Already been determined
   			...
   			$this->num_of_pages = $_GET['num_pages'];
      			...
      			if ($this->sql_query_string_count > $this->res_per_page) { // More than 1 page.
	...
	protected function get_start_limit(){
		...
		if(isset($_GET['start_limit']) && is_numeric($_GET['start_limit']) && ($_GET['start_limit'] > 0) ) { // Determine where in the database to start returning results...
	...
	protected function num_links_per_page($current_page, $num_links_to_show, $total_num_pages, $link_array){
   		...
   		if($num_links_to_show >= $total_num_pages){$num_links_to_show = $total_num_pages;}
